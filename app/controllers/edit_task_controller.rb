class EditTaskController < UIViewController
  ...
  def viewDidLoad
    ...
    @due_date_field.text = @task.due_date
    ...
    edit_task_button = UIButton.buttonWithType(UIButtonTypeSystem)
    edit_task_button.setTitle("Update Task", forState: UIControlStateNormal)
    edit_task_button.setTitleColor(UIColor.blackColor, forState: UIControlStateNormal)
    edit_task_button.titleLabel.font = UIFont.fontWithName("AmericanTypewriter", size: 20)
    edit_task_button.backgroundColor = UIColor.colorWithRed(188.0/255.0, green:225.0/255.0, blue:255.0/255.0, alpha:1.0)
    edit_task_button.frame = [[self.view.frame.size.width / 2 - self.view.frame.size.width * 3/4 / 2, self.view.frame.size.height / 10 * 7 + nav_bar_height], [self.view.frame.size.width * 3/4, self.view.frame.size.height / 10]]
    edit_task_button.layer.cornerRadius = 10
    self.view.addSubview(edit_task_button)
    edit_task_button.addTarget(self, action: "edit_task", forControlEvents: UIControlEventTouchUpInside)
